...
export const CpalRecorderServiceLive: RecorderService = {
	...
	stopRecording: async ({
		...
		if (stopRecordingError) {
			...
			if (audioRecording.audioData?.length) {
				sendStatus({
					title: '🧠 Finalizing Recording',
					description: 'Preparing your in-memory recording for transcription...',
				});

				primaryResult = Ok(
					audioDataToBlob({
						audioData: audioRecording.audioData,
						sampleRate: audioRecording.sampleRate,
						channels: audioRecording.channels,
					}),
				);
			} else if (!audioRecording.filePath) {
				...
				const { data: blob, error: readRecordingFileError } =
					await FsServiceLive.pathToBlob(audioRecording.filePath);
...
function audioDataToBlob({
	...
	const dataSize = samples.length * Float32Array.BYTES_PER_ELEMENT;
