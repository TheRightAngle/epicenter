...
/**
 * Fork-only preferences (TheRightAngle Windows-focused fork):
 *   - `output.fastMode`: skip slower post-processing steps after transcription
 *   - `system.minimizeToTray`: hide to tray on close rather than exit
 *   - `notifications.toastVisibility`: filter how many toasts to surface
 */
const forkPreferences = {
	'output.fastMode': defineKv(type('boolean'), false),
	'system.minimizeToTray': defineKv(type('boolean'), false),
	'notifications.toastVisibility': defineKv(
		type.enumerated('all', 'important-only', 'off'),
		'all',
	),
} as const;
