...
unsafe extern "system" fn global_hotkey_proc(
    ...
    if msg == WM_HOTKEY {
        ...
        // FORK PATCH (TheRightAngle/epicenter): upstream 0.7.0 spawns a
        // thread that busy-loops GetAsyncKeyState with no sleep/yield. For
        // push-to-talk — where the user holds the hotkey for 1–30s — that
        // pegs a CPU core for the entire hold. Add a 10ms sleep so release
        // detection runs at 100 Hz instead of ∞. Latency delta is
        // imperceptible; CPU usage drops from ~100% of one core to ~0%.
